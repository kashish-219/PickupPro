...
export function registerRatingsRoute() {
  route('/ratings/pending', requireAuth(async () => {
    ...
    render(`
      <section class="section"><div class="container" style="max-width:600px">
        ...
        ${pendingRatings.length?pendingRatings.map(pr=>`
          ...
          </div>`).join(''):`<div class="empty-state"><div class="empty-state__icon">⭐</div><h3>All done!</h3><p>No pending ratings</p><a href="/games" class="btn btn--primary" data-link>Browse Games</a></div>`}
      </div></section>`);
    $$('.star').forEach(s=>s.onclick=()=>{
      const v=+s.dataset.v;const box=s.closest('.rate-box');
    ...
    $$('.submit-rating').forEach(btn=>btn.onclick=async()=>{
      const box=btn.closest('.rate-box');const score=+box.dataset.rating;
      if(!score){toast('warning','Select a rating');return;}
      btn.disabled=true;btn.classList.add('btn--loading');
      try{await Ratings.create({gameId:box.dataset.game,toUserId:box.dataset.player,score,comment:box.querySelector('.comment').value});toast('success','Rated! ⭐');box.innerHTML='<div style="text-align:center;padding:var(--space-4);color:var(--success)">✅ Submitted!</div>';}
