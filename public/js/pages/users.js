...
import { PlayerCard, GameCard, SportSelector, setupSportSelector } from '../modules/components.js';
...
export function registerUserRoutes() {
  ...
  route('/players', async () => {
    ...
    async function load(){
      ...
      try{const r=await Users.list({search:$('#searchName').value,sport:$('#filterSport').value,limit:20});list.innerHTML=r.data.users.length?r.data.users.map(PlayerCard).join(''):'<div class="empty-state" style="grid-column:1/-1"><div class="empty-state__icon">👥</div><h3>No players found</h3></div>';}
  ...
  route('/profile', requireAuth(async () => {
    ...
    $('#profileForm').onsubmit=async e=>{
      ...
      try{await Users.update(currentUser._id,{name:fd.get('name'),bio:fd.get('bio'),sports:fd.getAll('sports')});currentUser.name=fd.get('name');updateAuthUI();toast('success','Saved! ✅');}
  ...
  route('/my-games', requireAuth(async () => {
    ...
    async function load(){
      ...
      list.innerHTML='<div class="loader" style="grid-column:1/-1"><div class="loader__ball">🏀</div></div>';
      try{const p={limit:50};if(f!=='all')p.role=f;const r=await Users.games(currentUser._id,p);list.innerHTML=r.data.games.length?r.data.games.map(GameCard).join(''):'<div class="empty-state" style="grid-column:1/-1"><div class="empty-state__icon">🎮</div><h3>No games</h3><a href="/games" class="btn btn--primary" data-link>Browse</a></div>';}
