...
export function registerGamesRoutes() {
      ...
      $("#createForm").onsubmit = async (e) => {
        ...
        try {
          const r = await Games.create({
            ...
            title: fd.get("title"),
            location: {
              name: fd.get("locationName"),
              city: fd.get("locationCity"),
              address: fd.get("locationAddress"),
            },
            date: fd.get("date"),
            minPlayers: +fd.get("minPlayers"),
  ...
  route(
    "/games/:id/edit",
    requireAuth(async ({ id }) => {
      const r = await Games.get(id);
      const g = r.data;
      if (g.hostId !== currentUser._id) {
        toast("error", "Not authorized");
        navigate("/games/" + id);
        return;
      }
      const dv = new Date(g.date);
