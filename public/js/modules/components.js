 ...
 */
import { SPORTS, getSport, escape, formatDate, formatTime, $$ } from './utils.js';
...
export function setupSportSelector() {
  $$('.sport-item').forEach(label => {
    label.onclick = e => {
      ...
      if (e.target !== input) { if (isRadio) input.checked=true; else input.checked=!input.checked; }
      if (isRadio) { $$('.sport-item').forEach(l=>l.classList.remove('active')); label.classList.add('active'); }
      else { label.classList.toggle('active', input.checked); }
    };
  });
}
